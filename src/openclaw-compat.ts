...
type SdkExports = {
  loadOutboundMediaFromUrl?: typeof loadOutboundMediaFromUrl;
  detectMime?: typeof detectMime;
  getDefaultMediaLocalRoots?: typeof getDefaultMediaLocalRoots;
};

const _sdkReady: Promise<SdkExports> = import("openclaw/plugin-sdk")
  .then((sdk) => {
    const exports: SdkExports = {};
    if (typeof (sdk as any).loadOutboundMediaFromUrl === "function") {
      exports.loadOutboundMediaFromUrl = (sdk as any).loadOutboundMediaFromUrl;
    }
    if (typeof (sdk as any).detectMime === "function") {
      exports.detectMime = (sdk as any).detectMime;
    }
    if (typeof (sdk as any).getDefaultMediaLocalRoots === "function") {
      exports.getDefaultMediaLocalRoots = (sdk as any).getDefaultMediaLocalRoots;
    }
    return exports;
  })
  .catch(() => {
...
async function detectMimeFallback(opts: DetectMimeOptions): Promise<string | undefined> {
  ...
  const isGeneric = (m: string | undefined): boolean =>
 ...
 */
export async function detectMime(bufferOrOpts: Buffer | DetectMimeOptions): Promise<string | undefined> {
...
async function assertLocalMediaAllowed(mediaPath: string, localRoots?: readonly string[]): Promise<void> {
...
/** 从远程 URL 获取媒体 */
async function fetchRemoteMedia(url: string, maxBytes?: number): Promise<{
  buffer: Buffer;
  contentType?: string;
  fileName?: string;
}> {
