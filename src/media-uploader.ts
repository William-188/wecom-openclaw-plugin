...
export async function uploadAndReplyMedia(
  ...
  try {
    // 1. 加载媒体文件
    log?.(`[wecom] Uploading media for reply: url=${mediaUrl}`);
    const media = await resolveMediaFile(mediaUrl, mediaLocalRoots);
    ...
    log?.(`[wecom] Media uploaded for reply: media_id=${uploadResult.media_id}, type=${finalType}`);

    // 5. 通过被动回复通道发送媒体消息
    const result = await wsClient.replyMedia(frame, finalType, uploadResult.media_id);
    const messageId = result?.headers?.req_id ?? `wecom-media-reply-${Date.now()}`;
